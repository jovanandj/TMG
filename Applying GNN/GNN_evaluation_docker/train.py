...
def test_model(model, test_graph, test_batches):
    ...
    with torch.no_grad():
        ...
        for cls in classes:
            ...
            cls_accuracy = accuracy_score(np.array(all_labels)[cls_mask], np.array(all_predictions)[cls_mask])
            class_accuracies.append(cls_accuracy)
        macro_accuracy = np.mean(class_accuracies)

    return accuracy, macro_accuracy, f1_macro, precision_macro, recall_macro, f1_weighted, precision_weighted, recall_weighted