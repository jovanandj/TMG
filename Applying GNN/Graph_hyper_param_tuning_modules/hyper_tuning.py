...
def main():
    ...
    storage = optuna.storages.RDBStorage("mysql+mysqlconnector://root:mojsql@db:3306/hyper_tuning_db")
    ...
    # Use 50% of the available CPUs
    n_jobs = math.floor(total_cpus * 0.4)

    logging.info(f'Number of CPUs: {total_cpus}')
    ...
    study.optimize(lambda trial: objective(trial, study, train_graph, validation_graph, train_nodes, validation_nodes),
                    n_jobs=n_jobs, n_trials=N_TRIALS, show_progress_bar=True, timeout = 3600)
